<template>
  ...
  <div class="md:max-w-xs mt-2 mx-auto md:mx-0 w-11/12 md:mt-0 md:border-r md:w-3/12 dark:border-gray-500">
      <!-- 手機：顯示收闔按鈕 -->
      <button
      class="md:hidden w-full px-4 py-4 bg-gray-200 rounded mb-2"
      ...
      :class="[
          ...
          <li>
              <button 
                  class="btn w-full hover:bg-gray-300 dark:hover:bg-gray-600 rounded-none" 
                  :class="{ 'bg-gray-300 dark:bg-gray-500': activeCategory === '' }"
                  @click="setSelectedCategory()">
                  顯示全部
              </button>
          </li>
          <li v-for="(item, index) in pluginStore.category" :key="index">
              <button class="btn w-full hover:bg-gray-300 dark:hover:bg-gray-600 rounded-none" 
              :class="{ 'bg-gray-300 dark:bg-gray-500': activeCategory === item }"
              @click="setSelectedCategory(item)">{{ item
              }}</button>
          </li>
      </ul>
      <button type="button" class="btn w-full text-center md:mb-10 hover:bg-gray-300 dark:hover:bg-gray-600 text-black rounded-none" 
...
<script setup>
  ...
  const themeClasses = computed(() => {
    return {
      text800: `text-${props.themeColor}-800`,
      bg300: `bg-${props.themeColor}-300`,
      hover: `hover:bg-${props.themeColor}-300`,
    };
  });
